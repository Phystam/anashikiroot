...
void TAlBigRIPSExample::Fill()
{
  ...
  if(pla){
    F3PLA_TL = pla->GetTLCal(); F3PLA_TR = pla->GetTRCal();
  ...
  if(pla){
    F7PLA_TL = pla->GetTLCal(); F7PLA_TR = pla->GetTRCal();
  ...
  if(pla){
    F8PLA_TL = pla->GetTLCal(); F8PLA_TR = pla->GetTRCal();
  ...
  if(pla){
    F11PLA_TL = pla->GetTLCal(); F11PLA_TR = pla->GetTRCal();
  ...
  if(isAnalyzeSAMURAI){
    ...
    if(pla){
      F13PLA1_TL = pla->GetTLCal(); F13PLA1_TR = pla->GetTRCal();
      F13PLA1_QL = pla->GetQLRaw(); F13PLA1_QR = pla->GetQRRaw(); 
    }
    // F13-Plastic-2
    pla = cpla->FindPlastic((char*)"F13pl-2");
    if(pla){
      F13PLA2_TL = pla->GetTLCal(); F13PLA2_TR = pla->GetTRCal();
