...
bool TAlEncPlasticExample::Calculate()
{
  ...
  for(int i=0;i<pla_paras->GetEntries();i++){
	...
	fAnaLoop->Add(PLASTIC, QRAW,   i+1, sqrt(pla->GetQLRaw() * pla->GetQRRaw()));
      ...
      fAnaLoop->Add(PLASTIC, TL,     i+1, pla->GetTLCal());
      fAnaLoop->Add(PLASTIC, TR,     i+1, pla->GetTRCal());
	...
	fAnaLoop->Add(PLASTIC, T,      i+1, (pla->GetTLCal() + pla->GetTRCal())/2.);
	fAnaLoop->Add(PLASTIC, TDIFF,  i+1, pla->GetTLCal() - pla->GetTRCal());
      }
