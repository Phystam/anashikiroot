...
bool TAlEncSAMURAIICExample::Calculate()
{
  ...
  if(ics){
    ...
    if(ic){
      ...
      fAnaLoop->Add(SAMURAIIC, ARawSqSum, 1, ic->GetPedADCSqSum());
      fAnaLoop->Add(SAMURAIIC, ARawAvSum, 1, ic->GetPedADCAvSum());
	...
	fAnaLoop->Add(SAMURAIIC, ARawSqSum, 2+ii, ic->GetPedADCSqSum());
	fAnaLoop->Add(SAMURAIIC, ARawAvSum, 2+ii, ic->GetPedADCAvSum());
