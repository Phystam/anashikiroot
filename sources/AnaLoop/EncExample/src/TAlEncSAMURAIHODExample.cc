...
bool TAlEncSAMURAIHODExample::Calculate()
{
  ...
  for(int i=0;i<hodplas->GetEntries();i++){
	...
	dtcal = pla->GetTUCal()-pla->GetTDCal();
	dtslw = pla->GetTUSlw()-pla->GetTDSlw();
      }
    }
	...
	fAnaLoop->Add(SAMURAIHOD, QAveCalTExist, id, pla->GetQAveCal());
      ...
      fAnaLoop->Add(SAMURAIHOD, TUCal,   id, pla->GetTUCal());
      fAnaLoop->Add(SAMURAIHOD, TDCal,   id, pla->GetTDCal());
      fAnaLoop->Add(SAMURAIHOD, TAveCal, id, pla->GetTAveCal());
      fAnaLoop->Add(SAMURAIHOD, TUSlw,   id, pla->GetTUSlw());
      fAnaLoop->Add(SAMURAIHOD, TDSlw,   id, pla->GetTDSlw());
      fAnaLoop->Add(SAMURAIHOD, TAveSlw, id, pla->GetTAveSlw());
