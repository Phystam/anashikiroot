  ...
  for (; node; node = node->GetNextNode()) {
    if (node->GetNodeType() == TXMLNode::kXMLElementNode) { // Element Node
      ...
      if (strcmp(node->GetNodeName(), "TOF") == 0)
        listOfTOFPara->Add(ParseTOFPara(node->GetChildren()));
    }
  }
  return;
}
...
TArtPlasticPara *TArtBigRIPSParameters::ParsePlasticPara(TXMLNode *node) {
  ...
  Double_t  tslew_left_c = 0;
  Double_t  tslew_right_a = 0;
  Double_t  tslew_right_b = 0;
  Double_t  tslew_right_c = 0;
  Double_t  tslew_ave_a = 0;
  Double_t  tslew_ave_b = 0;
  Double_t  tslew_ave_c = 0;
  Double_t  tdc_underflow = 0;
  Double_t  tdc_overflow = 4000;
  Double_t  zpos = 0;
  Double_t  toff = 0;
  ...
  for ( ; node; node = node->GetNextNode()) {
	...
	tslew_left_b = (Double_t)atof(node->GetText());
      if (strcmp(node->GetNodeName(),"tslew_left_c")==0)
	tslew_left_c = (Double_t)atof(node->GetText());
	...
	tslew_right_b = (Double_t)atof(node->GetText());
      if (strcmp(node->GetNodeName(),"tslew_right_c")==0)
	tslew_right_c = (Double_t)atof(node->GetText());
      if (strcmp(node->GetNodeName(),"tslew_ave_a")==0)
	tslew_ave_a = (Double_t)atof(node->GetText());
      if (strcmp(node->GetNodeName(),"tslew_ave_b")==0)
	tslew_ave_b = (Double_t)atof(node->GetText());
      if (strcmp(node->GetNodeName(),"tslew_ave_c")==0)
	tslew_ave_c = (Double_t)atof(node->GetText());
	...
	zpos = (Double_t)atof(node->GetText());
      if (strcmp(node->GetNodeName(), "xcal0") == 0)
	xcal0 = (Double_t)atof(node->GetText());
      if (strcmp(node->GetNodeName(), "xcal1") == 0)
	xcal1 = (Double_t)atof(node->GetText());
      if (strcmp(node->GetNodeName(), "xcal2") == 0)
	xcal2 = (Double_t)atof(node->GetText());
  ...
  TArtPlasticPara * para = new TArtPlasticPara(id, name, fpl, tcal_left, tcal_right, qped_left, qped_right, tslew_left_a, tslew_left_b, tslew_left_c, tslew_right_a, tslew_right_b, tslew_right_c,tslew_ave_a, tslew_ave_b, tslew_ave_c, tdc_underflow, tdc_overflow, zpos, toff, xcal0, xcal1, xcal2);
...
//__________________________________________________________
TArtTOFPara *TArtBigRIPSParameters::ParseTOFPara(TXMLNode *node) {

  TString upstreamplname;
  TString downstreamplname;
  TString name;
  Double_t offset;
  Int_t upstreamfpl;
  Int_t middlefpl;
  ...
  for ( ; node; node = node->GetNextNode()) {
    if (node->GetNodeType() == TXMLNode::kXMLElementNode) { // Element Node
      if (strcmp(node->GetNodeName(), "UpstreamPlName") == 0) 
	upstreamplname = (TString)node->GetText();
      if (strcmp(node->GetNodeName(), "DownstreamPlName") == 0) 
	downstreamplname = (TString)node->GetText();
      if (strcmp(node->GetNodeName(), "Offset") == 0) 
	offset = (Double_t)atof(node->GetText());
      if (strcmp(node->GetNodeName(), "MiddleFpl") == 0)
	middlefpl = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "UpstreamFpl") == 0)
	upstreamfpl = atoi(node->GetText());
      if (strcmp(node->GetNodeName(), "DownstreamFpl") == 0)
	downstreamfpl = atoi(node->GetText());

    }
  }
  name = Form("TOF%sto%s",upstreamplname.Data(),downstreamplname.Data());  
  TArtCore::Info(__FILE__,"Reading database for %s TOF",name.Data());
  TArtTOFPara * para = new TArtTOFPara(name.Data(), upstreamplname.Data(), downstreamplname.Data(), offset,
				       middlefpl, upstreamfpl, downstreamfpl );

  tof_pmap.insert(std::pair<TString, TArtTOFPara *>(name, para));
  return para;
