...
void TArtCalibIC::ReconstructData()   { // call after the raw data are loaded
  ...
  for(Int_t i=0;i<GetNumIC();i++){
    ...
    for(Int_t ii=0;ii<nlayer;ii++) {
      Double_t adc = (Double_t)ic->GetRawADC(ii);
      //if(adc>200) { //Set the threshold to higher values to avoid all the junk.
      if(adc>0) {
	Double_t pedadc =  adc - (Double_t)para->GetPedestal(ii);
	fAvSum += pedadc;
	fSqSum *= pedadc;
	fNumberOfChannelsFired ++;
	ic->SetEnergy(ii,para->GetCh2MeV(0) + para->GetCh2MeV(1)*((double)pedadc));
	ic->SetPedADC(ii, pedadc);
      }else{
	ic->SetEnergy(ii,TMath::QuietNaN());
	ic->SetPedADC(ii,TMath::QuietNaN());
      }
    }
    ...
    if(fNumberOfChannelsFired>0) {
      ...
      ic->SetPedADCAvSum(fAvSum);
      ic->SetPedADCSqSum(fSqSum);
