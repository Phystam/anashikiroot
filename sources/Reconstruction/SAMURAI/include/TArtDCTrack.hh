...
class TArtDCTrack : public TArtDataObject {
 ...
 public:
  ...
  virtual void AddSign(Double_t val){fsign.push_back(val);}
  ...
  virtual Double_t GetPosition(Int_t i=0){return cx[i];}//i=0 for x, i=1 for y
  // local angle at center z
  virtual Double_t GetAngle(Int_t i=0){return ca[i];}//i=0 for a, i=1 for b

  virtual Int_t GetHitPlaneID(Int_t i){return plane_id[i];}
  virtual Int_t GetHitID(Int_t i){return hitid[i];}
  virtual Int_t GetSign(Int_t hitid){return fsign[hitid];}
 ...
 private:
  ...
  std::vector<Int_t> fsign; // DC angle
 public:
