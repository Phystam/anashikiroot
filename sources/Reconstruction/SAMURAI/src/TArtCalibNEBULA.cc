...
void TArtCalibNEBULA::ReconstructData()
{
  ...
  for(Int_t i=0;i<GetNumNEBULAPla();++i){
    ...
    //乱数を振っておく
    Double_t turaw = pla->GetTURaw() + gRandom->Uniform(-0.5,0.5);
    Double_t tdraw = pla->GetTDRaw() + gRandom->Uniform(-0.5,0.5);
    Double_t quraw = pla->GetQURaw() + gRandom->Uniform(-0.5,0.5);
    Double_t qdraw = pla->GetQDRaw() + gRandom->Uniform(-0.5,0.5);
    ...
    Double_t tavecal = (tucal + tdcal)/2 - para->GetTAveOff() - para->GetTAveOffDelta();
    Double_t taveslw = (tuslw + tdslw)/2 
      - para->GetTAveOff() 
      - para->GetTAveOffSlw() 
      - para->GetTAveOffDelta() 
      + para->GetTAveOffLight();
    //    std::cout << para->GetTAveOffLight() << std::endl;
    Double_t poscal = dtcal * para->GetDTCal() + para->GetDTOff();
    Double_t posslw = dtslw * para->GetDTCal() + para->GetDTOff();
    //    std::cout << posslw << std::endl;
    ...
    if(para->GetSubLayer() != 0){ // NEUT
      pos[0] = para->GetDetPos(0) + posxoff + gRandom->Uniform(-60,60);
      //pos[0] = para->GetDetPos(0) + posxoff;
    }else{ // VETO
      pos[0] = para->GetDetPos(0) + posxoff + gRandom->Uniform(-160,160);
      //pos[0] = para->GetDetPos(0) + posxoff;
    }
    pos[1] = posslw + para->GetDetPos(1) + posyoff; 
    //pos[1] = poscal + para->GetDetPos(1) + posyoff; 
    if(para->GetSubLayer() != 0){      
      pos[2] = para->GetDetPos(2) + poszoff + gRandom->Uniform(-60,60);
    }else{
      pos[2] = para->GetDetPos(2) + poszoff + gRandom->Uniform(-5,5);
    }
