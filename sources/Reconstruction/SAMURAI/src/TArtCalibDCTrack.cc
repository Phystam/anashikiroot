...
void TArtCalibDCTrack::SetTDCDistribution(TH1 * inhtdc, Int_t layer_id) {
  ...
  if(tdcwindow[0]<0){ // in the case of v1190/v1290 case
	...
	tdc2mm[layer_id][i] = pitch[layer_id] * 0.5 * (Double_t)inhtdc->Integral(inhtdc->FindBin(i),inhtdc->FindBin(tdcwindow[1]))/tdcint[layer_id];
      }
    }
  }
}
...
Double_t TArtCalibDCTrack::STCCorrection(Double_t x, Double_t pitch, Double_t* c){
  x=x*2./pitch;
  Double_t guu = c[0]+c[1]*pow(x,2.)+c[2]*pow(x,4.);
  //  std::cout << x <<" " << pitch<<" "<< guu*x*(x-1.)*(x+1.) << std::endl;
  return x*(x-1.)*(x+1.)*guu;
}
//
//
//
...
TArtCalibDCTrack::Track1D * TArtCalibDCTrack::CalcTrack1D(std::vector <TArtDCHit *> *hits) {
  ...
  for(Int_t i=0;i<npattern;i++){
	...
	Double_t coef[]={(*hits)[l]->GetSTCCoef(0),(*hits)[l]->GetSTCCoef(1),(*hits)[l]->GetSTCCoef(2)};
    //    std::cout << dl << " ";
	dl[l] += STCCorrection(dl[l],pitch[lid[l]],coef);
