...
TArtNEBULAPlaPara *TArtSAMURAIParameters::ParseNEBULAPlaPara(TXMLNode *node)
{
  ...
  for(; node; node = node->GetNextNode()){
    ...
    }else if(strcmp(node->GetNodeName(), "TSlwPower") == 0){
      Double_t tslwpow = atof(node->GetText());
      para->SetTSlwPow(tslwpow);
    }else if(strcmp(node->GetNodeName(), "TAveOffSlw") == 0){
      Double_t taveoffslw = atof(node->GetText());
      para->SetTAveOffSlw(taveoffslw);
    }else if(strcmp(node->GetNodeName(), "TAveOffDelta") == 0){
      Double_t taveoffdelta = atof(node->GetText());
      para->SetTAveOffDelta(taveoffdelta);
    }else if(strcmp(node->GetNodeName(), "TAveOffLight") == 0){
      Double_t taveofflight = atof(node->GetText());
      para->SetTAveOffLight(taveofflight);
...
TArtNeuLANDPlaPara *TArtSAMURAIParameters::ParseNeuLANDPlaPara(TXMLNode *node)
{
  ...
  for(; node; node = node->GetNextNode()){
    ...
    if(strcmp(node->GetNodeName(), "ID") == 0){ 
      ...
      Double_t val = atof(node->GetText());
      para->SetAtt(val);
    }else if(strcmp(node->GetNodeName(), "tof_zero") == 0){
      Double_t val = atof(node->GetText());
      para->SetTOFZero(val);
...
TArtDCHitPara *TArtSAMURAIParameters::ParseDCHitPara(TXMLNode *node, std::map<TArtRIDFMap, TArtDCHitPara *> * map)
{
  ...
  Double_t coef0=0,coef1=0,coef2=0;
  Int_t det, geo, ch;
  ...
  for ( ; node; node = node->GetNextNode()) {
    if (node->GetNodeType() == TXMLNode::kXMLElementNode) { // Element Node
      ...
      if (strcmp(node->GetNodeName(), "coef0") == 0)
        coef0 = (Double_t)atof(node->GetText());
      if (strcmp(node->GetNodeName(), "coef1") == 0)
        coef1 = (Double_t)atof(node->GetText());
      if (strcmp(node->GetNodeName(), "coef2") == 0)
        coef2 = (Double_t)atof(node->GetText());
  ...
  TArtDCHitPara * para = new TArtDCHitPara(id, name, fpl, layer, id_plane, wireid, wirepos, wirez, tzero_offset,coef0,coef1,coef2);
